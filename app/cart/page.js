...
export default function CartPage() {
  ...
  const [address,      setAddress]      = useState({ fullName: '', phone: '', street: '', district: '', city: 'Cairo', notes: '' })
  ...
  async function placeOrder() {
    ...
    try {
      ...
      if (payMethod === 'card') {
        ...
        if (payData.paymentUrl) { saveCart([]); window.location.href = payData.paymentUrl; return }
  ...
  if (items.length === 0 && step === 'cart') {
    return (
      <>
        <Navbar />
        <div className="page-body" style={{ minHeight: 'calc(100vh - var(--nav-height))', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 1.5rem' }}>
  ...
  const renderSummary = (isCheckout) => (
    <div style={{ border: '1px solid var(--gray-200)', padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem', marginBottom: '1.25rem' }}>
        {items.map((it, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', fontSize: '0.75rem' }}>
            <span style={{ color: 'var(--gray-600)', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {it.name} × {it.qty || it.quantity || 1}
            </span>
            <span style={{ flexShrink: 0 }}>{formatPrice(it.price * (it.qty || it.quantity || 1))}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        {promo ? (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 0.75rem', background: '#f0faf0', border: '1px solid #c6eac6', fontSize: '0.72rem' }}>
            ...
            <button onClick={() => { setPromo(null); setPromoInput('') }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--gray-400)', fontSize: '0.65rem', fontFamily: 'inherit', padding: 0 }}>
              Remove
            </button>
          </div>
        ) : (
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <input
              ...
              style={{ flex: 1, border: '1px solid var(--gray-300)', padding: '0.55rem 0.75rem', fontSize: '0.75rem', fontFamily: 'inherit', outline: 'none', background: 'var(--white)', minWidth: 0 }}
            />
            <button
              onClick={applyPromo}
              disabled={promoLoading || !promoInput.trim()}
              className="btn btn-outline btn-sm"
              style={{ whiteSpace: 'nowrap', padding: '0.55rem 0.875rem', fontSize: '0.65rem', flexShrink: 0 }}
            >
              {promoLoading ? '…' : 'Apply'}
            </button>
          </div>
      ...
      {/* Totals */}
      <div style={{ borderTop: '1px solid var(--gray-200)', paddingTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.95rem', fontWeight: 600, paddingTop: '0.625rem', borderTop: '1px solid var(--gray-200)', marginTop: '0.125rem' }}>
          <span>Total</span><span>{formatPrice(total)}</span>
        </div>
      </div>

      {deliveryFee > 0 && (
        <p style={{ fontSize: '0.63rem', color: 'var(--sand)', marginTop: '0.75rem', letterSpacing: '0.02em' }}>
          Add {formatPrice(DELIVERY_THRESHOLD - subtotal)} more for free delivery
        </p>
      )}

      {/* CTA button — shown in summary only on desktop */}
      <div style={{ marginTop: '1.5rem' }} className="summary-cta">
        {!isCheckout ? (
          <button onClick={proceedToCheckout} className="btn btn-black btn-full" style={{ padding: '1.1rem', fontSize: '0.62rem', letterSpacing: '0.18em' }}>
            Proceed to Checkout
          </button>
        ) : (
          <button onClick={placeOrder} disabled={loading} className="btn btn-black btn-full" style={{ padding: '1.1rem', fontSize: '0.62rem', letterSpacing: '0.18em' }}>
      ...
      <p style={{ fontSize: '0.62rem', color: 'var(--gray-400)', textAlign: 'center', marginTop: '1rem', letterSpacing: '0.04em', lineHeight: 1.6 }}>
        Free delivery over EGP 500 · 2–3 days within Cairo
      </p>
    </div>
  )
  ...
  return (
    <>
      ...
      <div className="page-body">
        ...
        {/* Page header */}
        <div style={{ padding: 'clamp(1.5rem,4vw,3rem) clamp(1.25rem,4vw,3rem) 1.25rem', borderBottom: '1px solid var(--gray-300)' }}>
          <div className="section">
            <p className="t-label" style={{ marginBottom: '0.625rem' }}>
              {step === 'cart' ? 'Shopping Bag' : 'Checkout'}
            </p>
            <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.5rem,4vw,2.5rem)', fontWeight: 300, letterSpacing: '0.03em' }}>
            ...
            <div className="cart-main">
              ...
              {step === 'cart' ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'var(--gray-300)' }}>
                  {items.map((it, idx) => (
                    <div key={idx} style={{ background: 'var(--white)', padding: '1.25rem', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                      {/* Thumbnail */}
                      <div style={{ width: '76px', height: '96px', flexShrink: 0, background: 'var(--gray-100)', position: 'relative', overflow: 'hidden' }}>
                        {it.image && <Image src={it.image} alt={it.name} fill style={{ objectFit: 'cover' }} sizes="76px" />}
                      </div>
                      {/* Info */}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', marginBottom: '0.35rem' }}>
                          <span style={{ fontSize: '0.82rem', fontWeight: 500, lineHeight: 1.3 }}>{it.name}</span>
                          <span style={{ fontSize: '0.82rem', fontWeight: 500, whiteSpace: 'nowrap', flexShrink: 0 }}>
                            {formatPrice(it.price * (it.qty || it.quantity || 1))}
                          </span>
                        </div>
                        <p style={{ fontSize: '0.7rem', color: 'var(--gray-500)', marginBottom: '0.75rem' }}>
                          {[it.size && `Size ${it.size}`, it.color].filter(Boolean).join(' · ')}
                        </p>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                            <button onClick={() => updateQty(idx, -1)} style={{ background: 'none', border: '1px solid var(--gray-300)', width: '28px', height: '28px', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                            <span style={{ fontSize: '0.78rem', minWidth: '18px', textAlign: 'center' }}>{it.qty || it.quantity || 1}</span>
                            <button onClick={() => updateQty(idx, 1)} style={{ background: 'none', border: '1px solid var(--gray-300)', width: '28px', height: '28px', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                          </div>
                          <button onClick={() => remove(idx)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--gray-400)', fontFamily: 'inherit' }}>
                            Remove
                          </button>
                        </div>
                      </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      <div>
                        ...
                        <input className="input-line" value={address.fullName}
                          ...
                          placeholder="As on national ID" autoComplete="name" />
                      </div>
                      <div>
                        <label className="label">Phone Number</label>
                        <input className="input-line" value={address.phone}
                          onChange={e => setAddress(a => ({ ...a, phone: e.target.value }))}
                          placeholder="+20 1XX XXX XXXX" type="tel" autoComplete="tel" />
                      </div>
                      <div>
                        <label className="label">Street Address</label>
                        <input className="input-line" value={address.street}
                          onChange={e => setAddress(a => ({ ...a, street: e.target.value }))}
                          placeholder="Building number, street name" autoComplete="street-address" />
                      </div>
                      <div>
                        <label className="label">District / Area <span style={{ color: 'var(--gray-400)', fontWeight: 400 }}>(optional)</span></label>
                      ...
                      <div>
                        <label className="label">Special Instructions <span style={{ color: 'var(--gray-400)', fontWeight: 400 }}>(optional)</span></label>
                        ...
                        <label key={pm.id} style={{
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: '0.8rem', fontWeight: 500, marginBottom: '0.15rem', color: pm.disabled ? 'var(--gray-400)' : 'var(--black)' }}>
                              {pm.label}
                              {pm.disabled && (
                                <span style={{ marginLeft: '0.5rem', fontSize: '0.58rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gray-400)', fontWeight: 400 }}>
                                  Coming Soon
                                </span>
                              )}
                            </div>
                  ...
                  <button onClick={() => setStep('cart')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.62rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gray-500)', fontFamily: 'inherit', textAlign: 'left', padding: 0, width: 'fit-content' }}>
                    ← Back to Bag
                  </button>
                </div>
              )}
      ...
      <style>{`
        ...
        @media (max-width: 720px) {
          ...
          /* Tighter item padding on mobile */
          .cart-grid .cart-main > div > div {
            padding: 1rem !important;
          }

          /* Inputs full-size touch targets */
          .input-line {
            font-size: 16px !important; /* prevents iOS zoom on focus */
          }

          /* Payment labels easier to tap */
          .cart-grid label[style] {
