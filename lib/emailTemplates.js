  ...
  await sendMail({
    ...
    html: wrap(`
      ...
      <p style="color:#475569; line-height:1.6;">
        Your Diffuse buyer account is ready. Start browsing hundreds of clothing items from top sellers across the UAE.
      </p>
      <br>
...
export async function sendOrderConfirmation(to, order) {
  const itemRows = order.items?.map(item => `
    <tr>
      ...
      <td style="padding:8px; border-bottom:1px solid #E2E8F0; text-align:right;">AED ${item.priceAed}</td>
    </tr>
  `).join('') || ''
